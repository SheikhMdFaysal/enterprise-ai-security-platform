...
class LeakageDetector:
    ...
    LEAKAGE_PATTERNS = {
        ...
        "privilege_escalation": [
            r"(granted|giving|providing)\s+(you\s+)?(admin|administrator|elevated|superuser)\s+(access|privileges|permissions)",
            r"(switching|changed|upgraded)\s+(your\s+)?role\s+to",
            r"(access\s+controls?|permissions?|restrictions?)\s+(disabled|bypassed|overridden|removed)",
            r"you\s+now\s+have\s+(full|admin|unrestricted|elevated)\s+access",
            r"privilege\s+(escalation|elevation)\s+(successful|granted|complete)",
        ]
    }
