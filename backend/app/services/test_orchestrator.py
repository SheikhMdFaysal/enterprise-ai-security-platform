...
class TestOrchestrator:
    ...
    def create_test(
        ...
        for prompt_text in baseline_prompts:
            baseline = BaselinePrompt(security_test_id=test.id, prompt_text=prompt_text)
            db.add(baseline)
    ...
    def generate_variants_for_test(
        db: Session, test_id: int, count_per_technique: int = 2
    ) -> int:
    ...
    def execute_model_run(
        db: Session, variant_id: int, model_config: Dict[str, Any]
    ) -> ModelRun:
    ...
    def evaluate_run(db: Session, model_run_id: int) -> EvaluationScore:
        ...
        detection_result = LeakageDetector.detect_leakage(
            ...
            variant_text=model_run.style_variant.variant_text
            if model_run.style_variant
            else None,
        )
        ...
        confidence = (
            max(detection_result["confidence"].values())
