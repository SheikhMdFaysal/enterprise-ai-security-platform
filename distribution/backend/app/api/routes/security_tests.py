...
def list_security_tests(
    ...
    return {"tests": tests, "total": total, "limit": limit, "offset": offset}
...
def delete_security_test(test_id: int, db: Session = Depends(get_db)):
    ...
    return {"message": f"Test {test_id} deleted successfully", "test_id": test_id}
...
def cancel_security_test(test_id: int, db: Session = Depends(get_db)):
    ...
    if test.status == TestStatus.FAILED:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Test has already failed"
        )
...
def get_security_test(test_id: int, db: Session = Depends(get_db)):
    ...
    # Get scenario info
    scenario = (
        db.query(AttackScenario)
        .filter(AttackScenario.id == test.attack_scenario_id)
        .first()
    )
    ...
    for bp in test.baseline_prompts:
        ...
        baseline_prompts.append(
            {"id": bp.id, "prompt_text": bp.prompt_text, "variants": variants_data}
        )
...
def get_baseline_prompts(scenario_id: str):
    ...
    return {"scenario_id": scenario_id, "prompts": prompts, "count": len(prompts)}
...
def export_test_results(
    ...
    if format == "pdf":
        ...
        elements.append(
            Paragraph(f"<b>Target Vendor:</b> {test.target_vendor}", normal_style)
        )
        elements.append(
            Paragraph(f"<b>Target Model:</b> {test.target_model}", normal_style)
        )
        if test.created_at:
            elements.append(
                Paragraph(
                    f"<b>Created:</b> {test.created_at.strftime('%Y-%m-%d %H:%M')}",
                    normal_style,
                )
            )
        elements.append(
            Paragraph(
                f"<b>Export Date:</b> {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}",
                normal_style,
            )
        )
        elements.append(Spacer(1, 20))
        ...
        elements.append(
            Paragraph(
                f"Data Leakage Detected: {leaked_count} ({100 * leaked_count // max(total_runs, 1)}%)",
                normal_style,
            )
        )
        elements.append(
            Paragraph(
                f"Safe: {total_runs - leaked_count} ({100 * (total_runs - leaked_count) // max(total_runs, 1)}%)",
                normal_style,
            )
        )
        ...
        if results:
            ...
            for i, r in enumerate(results):
                ...
                table_data = [
                    ...
                    [
                        "Model",
                        f"{r.get('model_name', '')} ({r.get('model_vendor', '')})",
                    ],
                ...
                if r.get("leakage_categories"):
                    table_data.append(
                        ["Leakage Categories", r.get("leakage_categories")]
                    )
